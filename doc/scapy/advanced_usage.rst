...
.. seealso::
    ...
    :doc:`SLIP (Serial Line IP) <layers/slip>`
        Documentation of the :py:mod:`scapy.layers.slip` module, which contains
        a practical example of using a Packetizer.
...
.. tip::
    ...
    it is probably easier to implement a subclass of
    :py:class:`SLIPPacketizer`.
...
.. py:class:: Packetizer
    ...
    .. py:method:: encode_frame(packet: Union[bytes, Packet]) -> bytes
        ...
        end-users to encode a packet manually.  It is presumed to have no
        side-effects.
    ...
    .. py:attribute:: buffer (bytearray)
        ...
        with it must only be done by the holder of :py:attr:`.buffer_lock`, and
        must only be done with the class itself to ensure thread safety.

        *Subclasses must not write to this value.*

    .. py:attribute:: buffer_lock (threading.Lock)

        Protects use of :py:attr:`.buffer`. :py:class:`Packetizer`
        implementations should not need to interact with this.
    ...
    .. py:method:: find_end() -> int
        ...
        Return the length (in bytes) of the first packet in :py:attr:`.buffer`,
        or ``-1`` if there is no complete packet available.
...
.. py:class:: PacketizerSocket(SimpleSocket)
    ...
    queue (in addition to :py:attr:`Packetizer.buffer`). This is used if there
    is more than one packet that could fit into ``default_read_size`` (or a
    custom read size specified in ``recv`` or ``raw_recv``).
    ...
    .. py:method:: recv_raw([x: int = default_read_size]) -> Tuple[Optional[Type[Packet]], Optional[bytes], Optional[int]]
        ...
        There are some conditions that control whether a ``read`` is issued to
        the underlying stream:

        1.  If a Packet is available in the queue, it is returned immediately
            without reading from the underlying stream.

        2.  If no Packet is available in the queue, ``x`` bytes are read from
            the underlying stream, and then all of the packets are added to the
            internal queue.

        3.  Return the next packet in the queue, if it is available.
    ...
    .. py:method:: send(x: Union[Packet, bytes])
        ...
        Encodes a packet with :py:meth:`Packetizer.encode_frame` for
        transmission, and writes it to the underlying stream.

        If a :py:class:`Packet` of type :py:attr:`.packet_class` is passed,
        this is encoded and then written to the underlying stream.
