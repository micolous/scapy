...
class PacketizerSocket(SimpleSocket):
    ...
    def __init__(self, fd, packetizer, packet_class=None, packet_classes=None,
        ...
        default_read_size = (
            default_read_size if default_read_size is not None else 256)
        ...
        if packet_classes:
            packet_classes = set(packet_classes)
        else:
            packet_classes = set([self.packet_class])

        packet_classes.add(self.packet_class)
        packet_classes.add(binary_type)
        packet_classes.add(bytearray)
    ...
    def send(self, x):
        if isinstance(x, self.packet_classes):
            o = x
        else:
